...
class File extends EventEmitter {
  ...
  download(options, cb) {
    ...
    const maxChunkSize = options.maxChunkSize || 1024 * 1024 * 10
    ...
    this.api.request(req, (err, response) => {
      ...
      const getChunk = () => {
        ...
        const tryFetchChunk = () => {
          ...
          this.api.fetch(herokus[Math.floor(Math.random() * herokus.length)] + response.g + '/' + chunkOffset + '-' + chunkMax)
            .then(response => {
              handleMegaErrors(response)
  ...
  static fromURL(opt, extraOpt = {}) {
    if (extraOpt.urls) herokus = extraOpt.urls;
